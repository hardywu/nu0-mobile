...
export default class Pricing extends Component {
  ...
  componentDidMount() {
    let { bb } = this.state;
    let { currencyCoupleSelect, currencyCoupleDisplay } = this.state.bb;
    //发起币种请求
    api
      .getPublicCurrencies()
      .then(res => {
        ...
        currencyCoupleSelect.value.pricingCurId =
          currencyCoupleSelect.data[0].pricingCurId;
        this.setState({ bb: bb });
      })
      .catch(err => {
      ...
      .then(res => {
        ...
        for (let key in currencyCoupleSelect.marketsTickers) {
          if (reg.test(key)) {
            ...
            currencyCoupleDisplay.name = `${key
              .split(currencyCoupleSelect.value.pricingCurId)[0]
              .toUpperCase()}/${currencyCoupleSelect.value.pricingCurId.toUpperCase()}`;
            break;
          }
        }
        return {
  ...
  handleBbCoupleSelectTradeCoupleRelease = (evt, tradeCouple) => {
    let { bb } = this.state;
    let { currencyCoupleDisplay, currencyCoupleSelect } = this.state.bb;
    //高亮对应选项
  ...
  setBbTopPriceTopChange = marketId => {
    let { bb } = this.state;
    api
      .getPublicMarketsMarketTickers(marketId)
      .then(res => {
  ...
  render() {
    ...
    let { activeMainNavIndex, bb, lever, sideMenu } = this.state;
    return (
      <SafeAreaView style={[mStyles.mFlex1]}>
        ...
        <View style={styles.mainContainer}>
          {/* 币币内容开始 */}
          <View
            style={activeMainNavIndex === 0 ? mStyles.mFlex : mStyles.mHide}>
            {/* 交易开始 */}
              ...
              <View style={mStyles.mCenterContent}>
                <View style={styles.tradeHead}>
                  <View
                    ...
                    onResponderRelease={evt =>
                      this.handleBbCoupleValRelease(evt)
                    }>
                    <Text style={styles.tradeHeadTitle}>
                      {bb.currencyCoupleDisplay.name}
                    </Text>
                    <Image
                      style={styles.tradeHeadTitleArrow}
                      source={arrowIcon}
                    />
                  </View>
                  <View
                    style={[
                      styles.tradeHeadItem,
                      { justifyContent: 'flex-end' },
                    ]}>
                    <View style={styles.tradeHeadNumWrap}>
                      <Text
                        style={[styles.tradeHeadNumBig, mStyles.mGreenColor]}>
                        {bb.topPrice}
                      </Text>
                      {/* <Text style={styles.tradeHeadNumSmall}>$7385.234</Text> */}
                    </View>
                    <View style={[styles.tradeHeadQuoWrap, , mStyles.mGreenBg]}>
                      <Text style={styles.tradeHeadQuoText}>
                        {bb.topChange}
                      </Text>
                    </View>
                  </View>
                ...
                <View style={styles.tradeBody}>
                  <View style={styles.tradeBodyForm}>
                    <Form data={bb} setData={this.setBb} />
                  </View>
          ...
          {/* 杠杆内容开始 */}
          <View
            style={activeMainNavIndex === 1 ? mStyles.mFlex : mStyles.mHide}>
            {/* 交易开始 */}
            <View style={styles.trade}>
              <View style={mStyles.mCenterContent}>
                <View style={styles.tradeHead}>
                  <View
                    ...
                    onStartShouldSetResponder={() => true}
                    onResponderRelease={evt =>
                      this.handleLeverCoupleValRelease(evt)
                    }>
                ...
                <View style={styles.tradeBody}>
                  <View style={styles.tradeBodyForm}>
                    <Form data={lever} setData={this.setLever} />
                  </View>
                  <View style={styles.tradeBodyQuo}>
                    <List
